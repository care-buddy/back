...
export const getUserToken = async (
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: `토큰 검증 미들웨어 에러` });
  }
};
...
export const refreshToken = (
  ...
  passport.authenticate(
    'refresh',
    { session: false },
    async (err: Error, user: checkUser) => {
      if (err) {
        console.log('refresh authenticate 에러:', err);
        return res
          .status(500)
          .send({ message: `토큰 검증 미들웨어 에러: ${err.message}` });
      }
      ...
      try {
        const userRefreshToken = await userService.getUserRefreshToken(
          email as string,
        );
        ...
        if (userRefreshToken?.refreshToken === inputRefreshToken) {
          ...
          res.status(403).send({
            message: 'refresh 토큰이 만료되었거나 유효하지 않습니다.',
          });
        }
      } catch (error) {
